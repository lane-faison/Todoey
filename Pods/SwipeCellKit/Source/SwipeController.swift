...
class SwipeController: NSObject {
    ...
    @objc func handlePan(gesture: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            if (translation + originalCenter - swipeable.bounds.midX) * actionsView.orientation.scale > 0 {
                ...
                swipeable.actionsView?.visibleWidth = abs(swipeable.bounds.minX)
    ...
    func configureActionsView(with actions: [SwipeAction], for orientation: SwipeActionsOrientation) {
        ...
        if let visibleTableViewRect = delegate?.swipeController(self, visibleRectFor: scrollView) {
            let visibleSwipeableRect = swipeable.bounds.intersection(visibleTableViewRect)
            if visibleSwipeableRect.isNull == false {
                let top = visibleSwipeableRect.minY > swipeable.bounds.minY ? max(0, visibleSwipeableRect.minY - swipeable.bounds.minY) : 0
                let bottom = max(0, swipeable.bounds.size.height - visibleSwipeableRect.size.height - top)
